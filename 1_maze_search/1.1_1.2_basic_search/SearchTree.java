...
public class SearchTree {
	...
	//***Uniform cost with repeated state detection inside priority queue***//
	public boolean uniformCost(ReadMaze rm, STreeNode n, PriorityQueue pq) {
		pq.add(n);
		do {
		if (pq.isEmpty()) {
			return false;
		...
		if (rm.map[r][c] == '.') {
			...
			if (c != 0) {
				...
				if (rm.map[r][c-1] == ' ' || rm.map[r][c-1] == '.') {
					n.leftChild = new STreeNode(r,c-1,n); // Add to Tree
					updateMaxDepth(n.leftChild.depth);
					int var = n.leftChild.check(pq,"uniformCost");
					if (var == -1) {
						n.leftChild = null;
					}
					else if (var == 0) {
			...
			if (r != 0) {
				...
				if (rm.map[r-1][c] == ' ' || rm.map[r-1][c] == '.') {
					n.topChild = new STreeNode(r-1,c,n); // Add to Tree
					updateMaxDepth(n.topChild.depth);
					int var = n.topChild.check(pq,"uniformCost");
					if (var == -1) {
						n.topChild = null;
					}
					else if (var == 0) {
			...
			if (c != ((rm.map[0].length) - 1)) {
				...
				if (rm.map[r][c+1] == ' ' || rm.map[r][c+1] == '.') {
					n.rightChild = new STreeNode(r,c+1,n); // Add to Tree
					updateMaxDepth(n.rightChild.depth);
					int var = n.rightChild.check(pq,"uniformCost");
					if (var == -1) {
						n.rightChild = null;
					}
					else if (var == 0) {
			...
			if (r != ((rm.map.length) - 1)) {
				...
				if (rm.map[r+1][c] == ' ' || rm.map[r+1][c] == '.') {
					n.bottomChild = new STreeNode(r+1,c,n); // Add to Tree
					updateMaxDepth(n.bottomChild.depth);
					int var = n.bottomChild.check(pq,"uniformCost");
					if (var == -1) {
						n.bottomChild = null;
					}
					else if (var == 0) {
