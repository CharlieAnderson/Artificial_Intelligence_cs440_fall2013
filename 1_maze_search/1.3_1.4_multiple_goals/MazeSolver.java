...
public class MazeSolver {
	public static void main(String[] args) {
		String mazeToSolve = args[1];
		String algorithm = args[0];
		String mazeFile = "mazes/" + mazeToSolve + ".txt";
		String solnFile = "solutionMazes/"+algorithm+"_"+mazeToSolve+"Solution.txt";
		...
		if (algorithm.compareTo("dfs") == 0) {
			searchResult = st.dfs(rm,st.root,dq);
		}
		else if (algorithm.compareTo("bfs") == 0) {
			searchResult = st.bfs(rm,st.root,dq);
		}
			...
			searchResult = st.greedyBestAndAstar(rm,st.root,pq);
		}
		else if (algorithm.compareTo("astar") == 0) {
			Comparator<STreeNode> comparator = new STreeNodeComparatorAstar();
			PriorityQueue<STreeNode> pq = new PriorityQueue<STreeNode>(100, comparator);
			searchResult = st.greedyBestAndAstar(rm,st.root,pq);
		}

		if (searchResult) {st.solutionPath(rm);}
		...
		try {
			...
			for (int i=0; i< rm.map.length; i++) {
					for (int j=0; j< rm.map[0].length; j++) {
						pw.print(rm.map[i][j]);
						System.out.print((char)rm.map[i][j]);
					}
					pw.println();
					System.out.println();
			}
			pw.println("Path cost: "+st.finalGoal.cost);
			...
			System.out.println("Path cost: "+st.finalGoal.cost);
